...
CREATE OR REPLACE VIEW test_table AS
    ...
    FROM        test_table      c
;
ALTER TABLE ONLY test_table             ADD CONSTRAINT test_table_id_pkey                PRIMARY KEY (test_table_id);
ALTER TABLE ONLY test_table             ADD CONSTRAINT test_table_uniq                   UNIQUE (test_table_id);
ALTER TABLE ONLY test_table             ADD CONSTRAINT test_table_id_fkey                FOREIGN KEY (test_table_id)       REFERENCES coupon_serie (test_table_id)
                                                                                         ON UPDATE CASCADE ON DELETE CASCADE;
ALTER TABLE ONLY test_table             ADD CONSTRAINT test_table_id_fkey                FOREIGN KEY (client_id)           REFERENCES client (test_table_id)
                                                                                         ON UPDATE CASCADE ON DELETE SET NULL;
CREATE INDEX    test_table_id_idx                                                        ON test_table (test_table_id);
 */
